...
(() => {
  ...
  var require_buffer = __commonJS({
    "node_modules/buffer/index.js"(exports) {
      ...
      function writeFloat(buf, value, offset, littleEndian2, noAssert) {
        value = +value;
        offset = offset >>> 0;
        if (!noAssert) {
          checkIEEE754(buf, value, offset, 4, 34028234663852886e22, -34028234663852886e22);
        }
        ieee754.write(buf, value, offset, littleEndian2, 23, 4);
        return offset + 4;
      }
      ...
      function writeDouble(buf, value, offset, littleEndian2, noAssert) {
        value = +value;
        offset = offset >>> 0;
        if (!noAssert) {
          checkIEEE754(buf, value, offset, 8, 17976931348623157e292, -17976931348623157e292);
        }
        ieee754.write(buf, value, offset, littleEndian2, 52, 8);
        return offset + 8;
      }
  ...
  function _toHex(d8, { alphabet, scratchArr } = {}) {
    ...
    const len = d8.length, halfLen = len >>> 1, quarterLen = len >>> 2, out16 = scratchArr || new Uint16Array(len), d32 = new Uint32Array(d8.buffer, d8.byteOffset, quarterLen), out32 = new Uint32Array(out16.buffer, out16.byteOffset, halfLen), cc = alphabet === "upper" ? ccu : ccl;
    let i = 0, j = 0, v;
  ...
  function _toHexChunked(d, options = {}) {
    let hex = "", len = d.length, chunkWords = chunkBytes >>> 1, chunks = Math.ceil(len / chunkWords), scratchArr = new Uint16Array(chunks > 1 ? chunkWords : len);
  ...
  function _fromHex(s, { onInvalidInput, scratchArr, outArr, indexOffset } = {}) {
    if (!hl) {
      ...
      for (let l = 0; l < 22; l++) for (let r = 0; r < 22; r++) {
        const cl = l + (l < 10 ? 48 : l < 16 ? 55 : 81), cr = r + (r < 10 ? 48 : r < 16 ? 55 : 81), vin = littleEndian ? cr << 8 | cl : cr | cl << 8, vout = (l < 16 ? l : l - 6) << 4 | (r < 16 ? r : r - 6);
        hl[vin] = vout;
      }
    }
    const lax = onInvalidInput === "truncate", slen = s.length;
    if (!lax && slen & 1) throw new Error("Hex input is an odd number of characters");
    const bytelen = slen >>> 1, last7 = bytelen - 7, h16len = bytelen + 2, h16 = scratchArr || new Uint16Array(h16len), h8 = new Uint8Array(h16.buffer, h16.byteOffset), out = outArr || new Uint8Array(bytelen);
  ...
  function _fromHexChunked(s, { onInvalidInput } = {}) {
    const lax = onInvalidInput === "truncate", slen = s.length;
    if (!lax && slen & 1) throw new Error("Hex input is an odd number of characters");
    const byteLength = slen >>> 1, chunkInts = chunkBytes >>> 1, chunksCount = Math.ceil(byteLength / chunkInts), scratchArr = new Uint16Array((chunksCount > 1 ? chunkInts : byteLength) + 2), outArr = new Uint8Array(byteLength);
    for (let i = 0; i < chunksCount; i++) {
      const chunkStartByte = i * chunkInts, chunkEndByte = chunkStartByte + chunkInts, result = _fromHex(s.slice(chunkStartByte << 1, chunkEndByte << 1), {
        onInvalidInput,
        scratchArr,
  ...
  function _fromBase64(s, { alphabet, onInvalidInput } = {}) {
    ...
    const inBytes = te.encode(s), inBytesLen = inBytes.length, inIntsLen = inBytesLen >>> 2, inInts = new Uint32Array(inBytes.buffer, inBytes.byteOffset, inIntsLen), last3 = inIntsLen - 3, maxOutBytesLen = inIntsLen * 3 + inBytesLen % 4, outBytes = new Uint8Array(maxOutBytesLen), outInts = new Uint32Array(outBytes.buffer, 0, maxOutBytesLen >>> 2), wl = alphabet === "base64url" ? urlWordLookup : alphabet === "base64any" ? anyWordLookup : stdWordLookup, bl = alphabet === "base64url" ? urlByteLookup : alphabet === "base64any" ? anyByteLookup : stdByteLookup;
  ...
  function perf(log = console.log.bind(console)) {
    ...
    function benchmark(fn, iterations2) {
      ...
      for (let i = 0; i < iterations2; i++) fn();
      const t1 = performance.now();
      const t = (t1 - t0) / iterations2;
      const s = t.toFixed(2);
